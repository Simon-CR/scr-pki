...
class CAService:
    ...
    def get_acme_config(self) -> Dict[str, Any]:
        """Get current ACME configuration from Vault."""
        try:
            # Check if ACME is enabled
            acme_config = self.vault.client.secrets.pki.read_config_acme(mount_point='pki')
            enabled = acme_config['data'].get('enabled', False) if acme_config else False
            
            # Get cluster config to determine the URL base
            try:
                cluster_config = self.vault.client.secrets.pki.read_config_cluster(mount_point='pki')
                path = cluster_config['data'].get('path', '')
            except:
                path = ""
            
            # Construct directory URL
            # If path is http://vault:8200/v1/pki, directory is .../acme/directory
            directory_url = f"{path}/acme/directory" if path else ""
            
            return {
                "enabled": enabled,
                "directory_url": directory_url,
                "external_host": path  # Just returning the raw path for now
            }
        except Exception as e:
            logger.error("Failed to get ACME config", error=str(e))
            return {
                "enabled": False,
                "directory_url": "",
                "external_host": ""
    ...
    def update_acme_config(self, external_host: str) -> Dict[str, Any]:
        """
        Update ACME configuration.
        
        Args:
            external_host: The base URL for the Vault instance (e.g. http://192.168.1.50:8200)
        """
        try:
            # Ensure no trailing slash
            base_url = external_host.rstrip('/')
            
            # Update cluster path
            # We construct the path as {base_url}/v1/pki
            cluster_path = f"{base_url}/v1/pki"
            
            self.vault.client.secrets.pki.configure_cluster(
                mount_point='pki',
                path=cluster_path,
                aia_path=f"{base_url}/v1/pki"
            )
            
            # Ensure ACME is enabled
            self.vault.client.secrets.pki.configure_acme(
                mount_point='pki',
                enabled=True
            )
            
            # Also update URLs for certificates (AIA/CRL)
            self.vault.client.secrets.pki.configure_urls(
                mount_point='pki',
                issuing_certificates=[f"{base_url}/v1/pki/ca"],
                crl_distribution_points=[f"{base_url}/v1/pki/crl"]
            )
            
            return self.get_acme_config()
            
        except Exception as e:
            logger.error("Failed to update ACME config", error=str(e))
            raise e
